...
export default function DeleteCooperadoPage() {
  ...
  const handleDelete = async () => {
    try {
      const res = await fetch(`/api/cooperados/${params.id}`, { method: 'DELETE' });
      if (res.ok) {
    ...
    <div className="delete-container">
      <div className="card">
        <div className="header">
          <h1>Excluir Cooperado</h1>
        </div>
        <div className="content">
          <p>Tem certeza que deseja excluir o Cooperado:</p>
          <p className="item-name"><strong>Nome:</strong> {cooperado.name}</p>
