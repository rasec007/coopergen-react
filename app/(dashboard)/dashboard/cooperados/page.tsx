...
export default function CooperadosPage() {
  ...
  const handleDelete = async (id: string, name: string) => {
    if (!confirm(`Deseja realmente desativar o cooperado ${name}?`)) return;
    try {
      const res = await fetch(`/api/cooperados/${id}`, { method: 'DELETE' });
      if (res.ok) {
        setCooperados(cooperados.filter(c => c.id !== id));
      }
    } catch (error) {
  ...
  return (
    <div className="coop-container">
                  ...
                  <tr key={item.id}>
                    <td className="actions-cell">
                      <div className="actions-group-left">
                        ...
                        <Link href={`/dashboard/cooperados/${item.id}/delete`} className="action-btn delete" title="Excluir">
      ...
      <style jsx>{`
        ...
        .btn-add-circle:hover { background-color: #f1f5f9; border-color: #cbd5e1; }
        ...
        .main-name { font-weight: 600; color: #1e293b; font-size: 15px; }
        ...
        .action-btn { background: none; border: none; padding: 8px; border-radius: 8px; cursor: pointer; transition: all 0.2s; display: flex; align-items: center; justify-content: center; }
