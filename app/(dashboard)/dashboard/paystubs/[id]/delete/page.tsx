...
export default function DeletePaystubPage() {
  ...
  const handleDelete = async () => {
    try {
      const res = await fetch(`/api/paystubs/${params.id}`, { method: 'DELETE' });
      if (res.ok) {
    ...
    <div className="delete-container">
      <div className="card">
        <div className="header">
          <h1>Excluir Contra Cheque</h1>
        </div>
        <div className="content">
          <p>Tem certeza que deseja excluir o Contra Cheque:</p>
          <p className="item-name"><strong>Cooperado:</strong> {data.cooperado?.name}</p>
          <p className="item-name"><strong>Período:</strong> {data.month}/{data.year}</p>
          <p className="item-name"><strong>Tipo:</strong> {data.type}</p>
